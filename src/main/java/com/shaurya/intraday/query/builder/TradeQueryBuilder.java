...
public class TradeQueryBuilder {
  ...
  public static CustomQueryHolder queryToFetchDayTrades(String dayStartTime, String dayEndTime) {
    ...
    StringBuilder sb = new StringBuilder();
    sb.append(
        "select trade from Trade trade where trade.tradeDate between " + "'" + dayStartTime + "'"
            + " AND "
            + "'" + dayEndTime + "'");
    cq.setQueryString(sb.toString());
    cq.setInParamMap(new HashMap<>());
  ...
  public static CustomQueryHolder queryToFetchSecurityByName(String securityName) {
    CustomQueryHolder cq = new CustomQueryHolder();
    StringBuilder sb = new StringBuilder();
    sb.append("select ts from TradeStrategy ts where ts.securityName = '" + securityName + "'");
    cq.setQueryString(sb.toString());
    cq.setInParamMap(new HashMap<>());
    return cq;
  }
  ...
  public static CustomQueryHolder queryToUpdateTradeStock(String securityName) {
    CustomQueryHolder cq = new CustomQueryHolder();
    StringBuilder sb = new StringBuilder();
    sb.append(
        "update trade_strategy set day = 2 where security_name = '" + securityName + "'");
    cq.setQueryString(sb.toString());
    cq.setInParamMap(new HashMap<>());
    return cq;
  }

  public static CustomQueryHolder queryToUpdateStockBeta(String securityName, Double atr) {
    CustomQueryHolder cq = new CustomQueryHolder();
    StringBuilder sb = new StringBuilder();
    sb.append(
